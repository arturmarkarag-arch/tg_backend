 ...
 *   • orderNumber = next value from the shared atomic Product order-number lane
 *     (append to the catalog tail). No shiftUp, no use of originalOrderNumber.
 *     The worker decides shelving in Надходження.
  ...
  await withProductOrderNumberLock(async () => {
    ...
    try {
      await session.withTransaction(async () => {
        ...
        // Reserve the tail position through the same global allocator used by every
        // automatic Product creation path. The lane remains held until COMMIT.
        const newOrderNumber = await allocateProductOrderNumber();
