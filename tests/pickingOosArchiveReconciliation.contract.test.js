...
describe('OOS archive reconciliation contract', () => {
  it('consumes unreconciled OOS signals in the same archive transaction', () => {
    const source = fs.readFileSync(path.join(__dirname, '../services/archiveProductPrimitives.js'), 'utf8');
    expect(source).toContain('oosSignals.map((task) => task._id)');
  ...
  it('also heals an unreconciled signal when the product is already archived', () => {
    const archiveSource = fs.readFileSync(path.join(__dirname, '../services/archiveProductPrimitives.js'), 'utf8');
