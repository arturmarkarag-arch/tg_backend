...
describe('Products API', () => {
  it('creates a product via JSON and retrieves it', async () => {
    const createResponse = await request(app)
      .post('/api/products')
      .send({ orderNumber: 1, name: 'Test product', price: 100, quantity: 5 });
  ...
  it('creates a product via multipart form-data', async () => {
    ...
    expect(res.body.name).toBe('Multipart product');
  ...
  it('retrieves a single product by id', async () => {
    const created = await request(app)
      .post('/api/products')
      .send({ orderNumber: 1, name: 'Single', price: 10, quantity: 1 });
  ...
  it('updates a product', async () => {
    const created = await request(app)
      .post('/api/products')
      .send({ orderNumber: 1, name: 'Old name', price: 10, quantity: 1 });
  ...
  it('soft-deletes a product (archives it)', async () => {
    const created = await request(app)
      .post('/api/products')
      .send({ orderNumber: 1, name: 'To delete', price: 10, quantity: 1 });
  ...
  it('shifts later products after soft-deletion', async () => {
    await request(app).post('/api/products').send({ orderNumber: 1, name: 'First', price: 10, quantity: 1 });
    const second = await request(app).post('/api/products').send({ orderNumber: 2, name: 'Second', price: 12, quantity: 1 });
    await request(app).post('/api/products').send({ orderNumber: 3, name: 'Third', price: 14, quantity: 1 });
    ...
    expect(remaining.map((p) => p.name)).toEqual(['First', 'Third']);
    expect(remaining.map((p) => p.orderNumber)).toEqual([1, 2]);
  });

  it('shifts occupied order numbers when inserting a product', async () => {
    await request(app).post('/api/products').send({ orderNumber: 1, name: 'First', price: 10, quantity: 1 });
    await request(app).post('/api/products').send({ orderNumber: 2, name: 'Second', price: 12, quantity: 1 });

    const result = await request(app)
      .post('/api/products')
      .send({ orderNumber: 1, name: 'Inserted', price: 15, quantity: 1 });

    expect(result.status).toBe(201);
    const all = await Product.find().sort({ orderNumber: 1 });
    expect(all.map((p) => p.name)).toEqual(['Inserted', 'First', 'Second']);
    expect(all.map((p) => p.orderNumber)).toEqual([1, 2, 3]);
  });

  it('reorders products', async () => {
    const p1 = await request(app).post('/api/products').send({ orderNumber: 1, name: 'A', price: 1, quantity: 1 });
    const p2 = await request(app).post('/api/products').send({ orderNumber: 2, name: 'B', price: 1, quantity: 1 });
    ...
    expect(updated2.positionOrder).toBe(0);
    expect(updated1.positionOrder).toBe(1);
  });

  it('returns pending products sorted by orderNumber', async () => {
    await request(app).post('/api/products').send({ orderNumber: 2, name: 'Pending1', price: 5, quantity: 1, status: 'pending' });
    await request(app).post('/api/products').send({ orderNumber: 1, name: 'Pending2', price: 5, quantity: 1, status: 'pending' });
    await request(app).post('/api/products').send({ orderNumber: 3, name: 'Active1', price: 5, quantity: 1, status: 'active' });

    const res = await request(app).get('/api/products/pending');
    expect(res.status).toBe(200);
    expect(res.body.length).toBe(2);
    expect(res.body.map((item) => item.name)).toEqual(['Pending2', 'Pending1']);
  });
