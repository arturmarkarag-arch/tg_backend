...
The lightweight Intake membership refresh runs every 30 seconds by default. Because a terminal scan must walk the complete upstream status before applying `date_in_status`, Sent/Cancelled membership is refreshed separately at most once per five minutes (`BASELINKER_TERMINAL_REFRESH_MS`). Failed API calls put the scheduler into a ten-minute backoff (`BASELINKER_QUEUE_ERROR_BACKOFF_MS`) instead of retrying a blocked token every 30 seconds. Opening a terminal page uses one batched status/`id_from` read, not one exact request per card.
