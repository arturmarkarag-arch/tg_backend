...
let baseUrl = '';
const assertions = [];
const metrics = { api: {}, phases: {}, counts: {}, races: {}, chaos: {
  workerAhead: 0, sameBlockReentries: 0, blockSwitches: 0, pickerClaimConflicts: 0,
  lateJoins: 0, blockSizes: [], blockWorkers: {}, workerStats: {},
  telegramMockCalls: 0, telegramNetworkAttempts: 0, syntheticBotBlocked: 0,
} };
const CHAOS = {
  hotBlocks: Math.max(1, Math.min(CFG.blocks, Number.parseInt(process.env.LIVE_E2E_CHAOS_HOT_BLOCKS || '3', 10) || 3)),
  minDelayMs: Math.max(0, Number.parseInt(process.env.LIVE_E2E_CHAOS_MIN_DELAY_MS || '5', 10) || 0),
  maxDelayMs: Math.max(0, Number.parseInt(process.env.LIVE_E2E_CHAOS_MAX_DELAY_MS || '110', 10) || 0),
  lateJoinWorkers: Math.max(0, Number.parseInt(process.env.LIVE_E2E_CHAOS_LATE_JOIN || '2', 10) || 0),
  reentryBias: Math.max(0, Math.min(1, Number.parseFloat(process.env.LIVE_E2E_CHAOS_REENTRY_BIAS || '0.85') || 0)),
  minBarriers: Math.max(0, Number.parseInt(process.env.LIVE_E2E_CHAOS_MIN_BARRIERS || '2', 10) || 0),
  minReentries: Math.max(0, Number.parseInt(process.env.LIVE_E2E_CHAOS_MIN_REENTRIES || '1', 10) || 0),
  minSwitches: Math.max(0, Number.parseInt(process.env.LIVE_E2E_CHAOS_MIN_SWITCHES || '3', 10) || 0),
  minWorkersSameBlock: Math.max(1, Number.parseInt(process.env.LIVE_E2E_CHAOS_MIN_WORKERS_SAME_BLOCK || '3', 10) || 1),
};
...
const seedOverride = Number.parseInt(process.env.LIVE_E2E_CHAOS_SEED || '', 10);
const seedNum = Number.isFinite(seedOverride) ? (seedOverride >>> 0) : (Number.parseInt(crypto.createHash('sha1').update(RUN_ID).digest('hex').slice(0, 8), 16) >>> 0);
...
async function createWorld() {
  ...
  const rawChaosWeights = String(process.env.LIVE_E2E_CHAOS_BLOCK_WEIGHTS || '')
    .split(',').map((x) => Number.parseFloat(x.trim())).filter((n) => Number.isFinite(n) && n > 0);
  const chaosWeights = Array.from({ length: CFG.blocks }, (_, i) => rawChaosWeights[i] || (1 + ((i * 7) % 13)));
  let productCursor = 0;
  let remainingProducts = world.products.length;
  for (let b = 0; b < CFG.blocks; b += 1) {
    const remainingBlocks = CFG.blocks - b;
    const remainingWeight = chaosWeights.slice(b).reduce((sum, n) => sum + n, 0);
    let count = remainingBlocks === 1
      ? remainingProducts
      : Math.max(1, Math.round(remainingProducts * chaosWeights[b] / Math.max(1, remainingWeight)));
    count = Math.min(count, remainingProducts - (remainingBlocks - 1));
    const ids = world.products.slice(productCursor, productCursor + count).map((p) => p._id);
    productCursor += count;
    remainingProducts -= count;
    if (!ids.length) break;
    metrics.chaos.blockSizes.push(ids.length);
    world.blocks.push(await Block.create({ blockId: blockIds[b], productIds: ids, version: 1 }));
  }
  check(Math.max(...metrics.chaos.blockSizes) > Math.min(...metrics.chaos.blockSizes),
    'CHAOS physical blocks are deliberately uneven', JSON.stringify(metrics.chaos.blockSizes));
...
function chaosWorkerNumber(wh) {
  const m = String(wh.firstName || '').match(/(\d+)$/);
  return Math.max(1, Number.parseInt(m?.[1] || '1', 10) || 1);
}
function chaosNoteBlockWorker(blockId, wh) {
  const key = String(blockId);
  const name = String(wh.firstName || wh.telegramId);
  if (!metrics.chaos.blockWorkers[key]) metrics.chaos.blockWorkers[key] = [];
  if (!metrics.chaos.blockWorkers[key].includes(name)) metrics.chaos.blockWorkers[key].push(name);
}
function chaosChooseWeighted(rows) {
  if (!rows.length) return null;
  const weights = rows.map((row) => Math.max(1, Number(row.taskCount || 0)) ** 1.20);
...
async function chaosClaimFromPicker(wh, {
  ...
  for (let attempt = 0; attempt < 8; attempt += 1) {
    const overviewPath = '/api/picking/blocks-overview?deliveryGroupId=' + encodeURIComponent(str(world.group._id));
    const overview = await api('GET', overviewPath, wh, undefined, 'GET chaos-blocks-overview');
    check(overview.status === 200, 'CHAOS blocks-overview stays healthy', 'worker=' + wh.firstName + ' status=' + overview.status);
    const rows = (overview.data?.blocks || []).filter((b) => Number(b.taskCount || 0) > 0);
    if (!rows.length) return null;

    let chosen = null;
    const preferred = preferredBlockId == null ? null : rows.find((b) => Number(b.blockId) === Number(preferredBlockId));
    if (preferred) {
      const forceRequiredReentry = reason === 'barrier' && metrics.chaos.sameBlockReentries < CHAOS.minReentries;
      if (reason === 'initial' || forceRequiredReentry || rnd() < CHAOS.reentryBias) chosen = preferred;
    }
    if (!chosen) chosen = chaosChooseWeighted(rows);
    if (!chosen) return null;

    const pickerPath = '/api/picking/block-tasks?blockId=' + encodeURIComponent(chosen.blockId)
      + '&deliveryGroupId=' + encodeURIComponent(str(world.group._id));
    const picker = await api('GET', pickerPath, wh, undefined, 'GET chaos-block-tasks');
    check(picker.status === 200, 'CHAOS block-task picker stays healthy', 'worker=' + wh.firstName + ' block=' + chosen.blockId);
    let free = (picker.data?.tasks || []).filter((t) => t.status === 'pending' && !t.lockedByOther);
    if (reason === 'barrier' && Number(chosen.blockId) === Number(preferredBlockId) && Number(minPosition) > 0) {
      free = free.filter((t) => Number(t.positionIndex) >= Number(minPosition));
    }
    if (!free.length) {
      preferredBlockId = null;
      minPosition = 0;
    ...
    free.sort((a, b) => Number(a.positionIndex) - Number(b.positionIndex));
    let candidate;
    if (reason === 'initial' && Number(chosen.blockId) === Number(preferredBlockId)) {
      const workerIdx = chaosWorkerNumber(wh) - 1;
      const rank = Math.floor(workerIdx / Math.max(1, CHAOS.hotBlocks));
      const approxGroupSize = Math.max(1, Math.ceil(CFG.warehouses / Math.max(1, CHAOS.hotBlocks)));
      const fraction = approxGroupSize <= 1 ? 0 : Math.min(0.92, rank / approxGroupSize);
      candidate = free[Math.min(free.length - 1, Math.floor(fraction * free.length))];
    } else if (reason === 'barrier') {
      const from = Math.floor(free.length * 0.35);
      candidate = free[from + Math.floor(rnd() * Math.max(1, free.length - from))];
    } else {
      candidate = free[Math.floor(rnd() * free.length)];
    }

    const claimPath = '/api/picking/tasks/' + encodeURIComponent(candidate.taskId) + '/claim';
    const claim = await api('POST', claimPath, wh, {}, 'POST chaos-picker-claim');
    if (claim.status === 200 && claim.data?.task) {
      chaosNoteBlockWorker(chosen.blockId, wh);
      const stat = metrics.chaos.workerStats[wh.firstName] || { entries: 0, barriers: 0, reentries: 0, switches: 0, tasks: 0 };
      stat.entries += 1;
      if (reason === 'barrier' && Number(chosen.blockId) === Number(preferredBlockId)) {
        metrics.chaos.sameBlockReentries += 1;
        stat.reentries += 1;
      }
      if (previousBlockId != null && Number(chosen.blockId) !== Number(previousBlockId)) {
        metrics.chaos.blockSwitches += 1;
        stat.switches += 1;
      }
      metrics.chaos.workerStats[wh.firstName] = stat;
      return { task: claim.data.task, blockId: chosen.blockId };
    }
    if ([409, 423].includes(claim.status)) {
      metrics.chaos.pickerClaimConflicts += 1;
      await sleep(2 + Math.floor(rnd() * 20));
      continue;
    }
    check(false, 'CHAOS picker claim returns expected status',
      'worker=' + wh.firstName + ' status=' + claim.status + ' err=' + (claim.data?.error || ''));
  }
  return null;
}
...
async function runWorker(wh, initialBlockId, sessionId, outcomeSets, stop) {
  ...
  let task = null;
  let iterations = 0;
  let entryReason = 'initial';
  let barrierPosition = 0;
  const workerIdx = chaosWorkerNumber(wh) - 1;
  const rank = Math.floor(workerIdx / Math.max(1, CHAOS.hotBlocks));
  const approxGroupSize = Math.max(1, Math.ceil(CFG.warehouses / Math.max(1, CHAOS.hotBlocks)));
  const rankFraction = approxGroupSize <= 1 ? 0 : Math.min(1, rank / (approxGroupSize - 1));
  const baseDelay = Math.round(CHAOS.minDelayMs + rankFraction * Math.max(0, CHAOS.maxDelayMs - CHAOS.minDelayMs));
  const lateStart = workerIdx >= Math.max(0, CFG.warehouses - CHAOS.lateJoinWorkers);
  if (lateStart) {
    metrics.chaos.lateJoins += 1;
    await sleep(250 + Math.floor(rnd() * 1000));
  }

  while (!stop.value && iterations < CFG.products * 8) {
    iterations += 1;
    if (!task) {
      const entry = await chaosClaimFromPicker(wh, {
        preferredBlockId: blockId,
        minPosition: barrierPosition > 0 ? barrierPosition + 1 : 0,
        reason: entryReason,
        previousBlockId: blockId,
      });
      if (!entry) {
        const pending = await PickingTask.countDocuments({
          orderingSessionId: str(sessionId), deliveryGroupId: str(world.group._id), status: 'pending',
        });
        if (!pending) break;
        entryReason = 'dynamic';
        barrierPosition = 0;
        await sleep(5 + Math.floor(rnd() * 25));
        continue;
      }
      task = entry.task;
    ...
    if (baseDelay > 0 || CHAOS.maxDelayMs > 0) {
      const jitter = Math.floor(rnd() * Math.max(1, Math.floor((CHAOS.maxDelayMs + 1) / 5)));
      await sleep(baseDelay + jitter);
    }

    const currentTask = task;
    const currentBlock = currentTask.blockId || blockId;
    const result = await processTask(wh, currentTask, outcomeSets.oos, outcomeSets.short, outcomeSets.progress);
    check(result.status === 200, 'CHAOS task finalization succeeds',
      'worker=' + wh.firstName + ' task=' + tid(currentTask) + ' status=' + result.status + ' err=' + (result.data?.error || ''));
    const stat = metrics.chaos.workerStats[wh.firstName] || { entries: 0, barriers: 0, reentries: 0, switches: 0, tasks: 0 };
    stat.tasks += 1;
    metrics.chaos.workerStats[wh.firstName] = stat;

    if (result.data?.routeBlocked?.code === 'worker_ahead') {
      metrics.chaos.workerAhead += 1;
      stat.barriers += 1;
      task = null;
      blockId = currentBlock;
      barrierPosition = Number(result.data.routeBlocked.positionIndex || 0);
      entryReason = 'barrier';
      continue;
    }
    ...
    if (result.data?.nextTask) {
      ...
      blockId = task.blockId || currentBlock;
      barrierPosition = 0;
      entryReason = 'dynamic';
      continue;
    }
...
async function runMass() {
  ...
  const chaosHotBlocks = [...world.blocks].sort((a, b) => b.productIds.length - a.productIds.length).slice(0, Math.max(1, CHAOS.hotBlocks));
  check(chaosHotBlocks.length > 0, 'CHAOS has hot blocks for clustered worker starts');
  const workerPromises = world.warehouses.map((wh, i) => runWorker(
    wh,
    chaosHotBlocks[i % chaosHotBlocks.length].blockId,
    currentSessionId,
  ...
  const chaosWorkerCounts = Object.values(metrics.chaos.blockWorkers).map((rows) => new Set(rows).size);
  metrics.chaos.maxDistinctWorkersInOneBlock = chaosWorkerCounts.length ? Math.max(...chaosWorkerCounts) : 0;
  metrics.chaos.minBlockSize = metrics.chaos.blockSizes.length ? Math.min(...metrics.chaos.blockSizes) : 0;
  metrics.chaos.maxBlockSize = metrics.chaos.blockSizes.length ? Math.max(...metrics.chaos.blockSizes) : 0;
  check(metrics.chaos.workerAhead >= CHAOS.minBarriers, 'CHAOS produces real worker_ahead barriers under concurrent lanes',
    `actual=${metrics.chaos.workerAhead} required>=${CHAOS.minBarriers}`);
  check(metrics.chaos.sameBlockReentries >= CHAOS.minReentries, 'CHAOS proves same-block re-entry after worker_ahead',
    `actual=${metrics.chaos.sameBlockReentries} required>=${CHAOS.minReentries}`);
  check(metrics.chaos.blockSwitches >= CHAOS.minSwitches, 'CHAOS workers dynamically switch between physical blocks',
    `actual=${metrics.chaos.blockSwitches} required>=${CHAOS.minSwitches}`);
  check(metrics.chaos.maxDistinctWorkersInOneBlock >= CHAOS.minWorkersSameBlock, 'CHAOS has multiple workers sharing one block',
    `actual=${metrics.chaos.maxDistinctWorkersInOneBlock} required>=${CHAOS.minWorkersSameBlock}`);
  check(metrics.chaos.maxBlockSize > metrics.chaos.minBlockSize * 2, 'CHAOS block load is strongly uneven',
    `min=${metrics.chaos.minBlockSize} max=${metrics.chaos.maxBlockSize}`);

  metrics.chaos.telegramMockCalls = (global.__LIVE_E2E_TELEGRAM_MOCK_CALLS__ || []).length;
  metrics.chaos.telegramNetworkAttempts = (global.__LIVE_E2E_TELEGRAM_NETWORK_ATTEMPTS__ || []).length;
  const syntheticTgIds = [...world.sellers, ...world.warehouses]
    .map((u) => String(u?.telegramId || ''))
    .filter(Boolean);
  metrics.chaos.syntheticBotBlocked = syntheticTgIds.length
    ? await User.countDocuments({ telegramId: { $in: syntheticTgIds }, botBlocked: true })
    : 0;

  check(global.__LIVE_E2E_TELEGRAM_ISOLATED__ === true,
    'CHAOS outbound Telegram module is hard-isolated inside the E2E process');
  eq(metrics.chaos.telegramNetworkAttempts, 0,
    'CHAOS made ZERO real api.telegram.org network attempts');
  eq(metrics.chaos.syntheticBotBlocked, 0,
    'CHAOS synthetic sellers/workers never become botBlocked from test side-effects');
...
function writeReport(error = null) {
  ...
  const lines = [
    ...
    `- CHAOS hot blocks: ${CHAOS.hotBlocks}`,
    `- CHAOS block sizes: ${metrics.chaos.blockSizes.join(', ')}`,
    `- CHAOS worker_ahead barriers: ${metrics.chaos.workerAhead}`,
    `- CHAOS same-block reentries: ${metrics.chaos.sameBlockReentries}`,
    `- CHAOS block switches: ${metrics.chaos.blockSwitches}`,
    `- CHAOS picker claim conflicts: ${metrics.chaos.pickerClaimConflicts}`,
    `- Telegram mock calls (local/no-op): ${metrics.chaos.telegramMockCalls}`,
    `- Telegram REAL network attempts: ${metrics.chaos.telegramNetworkAttempts}`,
    `- Synthetic users botBlocked=true: ${metrics.chaos.syntheticBotBlocked}`,
