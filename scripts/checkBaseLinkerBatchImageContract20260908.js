...
const checks = [
  ...
  ['unlinked backlog scans its exact storage catalog in documented pages', products.includes('advanceStorageSnapshot') && products.includes("'getInventoryProductsList'") && products.includes("'getProductsList'") && products.includes('include_variants: true')],
  ['unlinked matches load detailed media in chunks instead of one row at a time', products.includes('loadMatchedProductImages') && products.includes('refsByProductId') && products.includes('for (const ids of chunk(')],
  ['marketplace HTML scraping is absent from the product image runtime', !products.includes('allegro.pl/oferta/') && !products.includes('fetchAllegroOfferImage')],
