...
check('FA(3) generator is provider-local and conservative', fa3.includes('FA3_NAMESPACE') && fa3.includes('ksef_stage3_correction_not_supported') && fa3.includes('ksef_stage3_currency_not_supported') && fa3.includes("new Set(['23', '8', '5'])"));
...
check('online session uses encrypted key + IV and sends encrypted invoice', online.includes("'/sessions/online'") && online.includes('encryptedSymmetricKey') && online.includes('initializationVector') && online.includes('encryptedInvoiceContent') && online.includes('offlineMode: false'));
...
check('offline, UPO and inbound capabilities stay explicitly disabled in Stage 3', adapter.includes('[CAPABILITIES.OFFLINE]: false') && adapter.includes('[CAPABILITIES.UPO]: false') && adapter.includes('[CAPABILITIES.RECEIVE]: false'));
