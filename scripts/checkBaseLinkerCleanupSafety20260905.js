...
check(shared.includes('baselinker.orderCache.v2'), 'order cache state is wiped');
check(shared.includes('baselinker.journal.v1'), 'journal cursor/state is wiped');
check(shared.includes('BaseLinkerOrderCache'), 'order mirror is wiped');
check(shared.includes('BaseLinkerOrderSnapshot'), 'raw snapshots are wiped');
...
check(shared.includes('await model.syncIndexes()'), 'single-account indexes are synchronized after wipe');
