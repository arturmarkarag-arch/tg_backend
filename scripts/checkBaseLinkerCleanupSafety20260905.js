...
check(shared.includes('baselinker.orderIndex.v1'), 'retired minimal order index state is wiped');
check(shared.includes('baselinker.orderIndex.v3'), 'current order index scheduler state is wiped');
...
check(shared.includes('BaseLinkerProductImageCache'), 'non-PII BaseLinker product image cache is wiped');
...
check(shared.includes('await model.syncIndexes()'), 'current account-scoped indexes are synchronized after wipe');
