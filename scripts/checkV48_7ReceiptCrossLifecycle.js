...
const checks = [
  ...
  ['batch claim rechecks confirmed/product/supplement state', /await ReceiptItem\.updateMany\([\s\S]*?status: 'confirmed'[\s\S]*?'routing\.supplement': true[\s\S]*?createdProductId: \{ \$ne: null \}[\s\S]*?supplementPublishRequestedAt: null/.test(receipts)],
  ['claimed batch rows are re-read with confirmed/product fence', /const selectedRows = await ReceiptItem\.find\(\{[\s\S]*?status: 'confirmed'[\s\S]*?createdProductId: \{ \$ne: null \}[\s\S]*?supplementPublishRequestedAt: now/.test(receipts)],
