...
async function scenarioConflictMove() {
  ...
  try {
    ...
    const conflictSession = await currentSession(world);
    const move = await api('POST', '/api/v1/orders/conflicts/resolve', wh, {
      ...
      deliveryGroupId: str(world.group._id), orderingSessionId: str(conflictSession._id),
    });
...
async function scenarioArchiveDuringOpenOrdering() {
  ...
  try {
    ...
    // Coverage is meaningful only AFTER start-session has built PickingTasks.
    // Before the builder runs, every legitimate live item is necessarily no_task.
    const coverage = await auditSessionCoverage({
      deliveryGroupId: str(world.group._id),
      orderingSessionId: sid,
    });
    check(coverage.ok, 'archive_open: cancelled archived line creates NO coverage gap after task build', `gaps=${JSON.stringify(coverage.gaps || [])}`);
