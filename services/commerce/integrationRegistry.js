...
const PROVIDERS = Object.freeze([
  ...
  },
  {
    ...
    api: [
      ...
      { id: 'offers.stock.preview', label: 'Preview залишку', operation: 'GET /sale/offers by external.id + Commerce Inventory/reservation policy', direction: 'read', implementation: LIVE, capability: 'saleOffersRead', scope: 'allegro:api:sale:offers:read', note: 'Stage 3D.6C: окремий Commerce Inventory onHand мінус provider-neutral Commerce reservations, далі channel buffer/cap. Основний склад Product.quantity не використовується як online stock. stock=0 завершує ACTIVE offer; ENDED не відновлюється простим збільшенням stock.' },
      ...
      { id: 'offers.stock.write', label: 'Синхронізація залишку', operation: 'POST /sale/offer-bulk-modification-commands (stock) + GET summary/tasks', direction: 'write', implementation: LIVE, capability: 'saleOffersWrite', scope: 'allegro:api:sale:offers:write', note: 'Stage 3D.6C: до 25 різних FIXED stock modifications/command через beta bulk resource. Source of truth тільки Commerce Inventory мінус central reservations; main warehouse Product.quantity не читаємо і не змінюємо. Durable commandId, polling/recovery, read-back verification; stock=0 потребує явного підтвердження, бо завершує ACTIVE offer.' },
      { id: 'offers.lifecycle.read', label: 'Lifecycle preview', operation: 'GET /sale/product-offers/{offerId} + Commerce stock readiness', direction: 'read', implementation: LIVE, capability: 'saleOffersRead', scope: 'allegro:api:sale:offers:read', note: 'Stage 3D.7A: read-only визначення ACTIVE/ENDED, endedBy, BUY_NOW reopen eligibility та необхідності stock preparation.' },
      { id: 'offers.lifecycle.write', label: 'END / REOPEN offer', operation: 'PUT /sale/offer-publication-commands/{commandId} + quantity command when ENDED stock must be prepared', direction: 'write', implementation: LIVE, capability: 'saleOffersWrite', scope: 'allegro:api:sale:offers:write', note: 'Stage 3D.7A: lifecycle окремий від Stock Sync. END/ACTIVATE через stable public command resource; для ENDED+stock mismatch спочатку PUT quantity-change FIXED, потім ACTIVATE. Durable client UUID, summary/tasks polling і read-back verification.' },
      { id: 'offers.health.read', label: 'Final health / reconciliation', operation: 'GET /sale/product-offers/{offerId} + GET /sale/offer-events', direction: 'read', implementation: LIVE, capability: 'saleOffersRead', scope: 'allegro:api:sale:offers:read', note: 'Stage 3D.7B: live read-back усіх керованих полів (identity/mapping/content/sales settings/price/stock/lifecycle/jobs) плюс offer-events як best-effort діагностика останніх 24 годин. Upstream write = 0.' },
    ],
  },
  {
    id: 'olx',
