...
const PROVIDERS = Object.freeze([
  ...
  },
  {
    ...
    api: [
      ...
      { id: 'offers.stock.preview', label: 'Preview залишку', operation: 'GET /sale/offers by external.id + warehouse/reservation policy', direction: 'read', implementation: LIVE, capability: 'saleOffersRead', scope: 'allegro:api:sale:offers:read', note: 'Stage 3D.6B: warehouse quantity мінус provider-neutral Commerce reservations, далі channel buffer/cap. stock=0 завершує ACTIVE offer; ENDED не відновлюється простим збільшенням stock.' },
      ...
      { id: 'offers.stock.write', label: 'Синхронізація залишку', operation: 'POST /sale/offer-bulk-modification-commands (stock)', direction: 'write', implementation: PLANNED, capability: 'saleOffersWrite', scope: 'allegro:api:sale:offers:write', note: 'Reservation ledger уже LIVE. Write лишається locked до наступного inventory movement/reconciliation контракту: shipped/consumed units мають бути однозначно пов’язані з тим, чи Product.quantity вже фізично зменшена/перерахована.' },
    ],
  },
  {
    id: 'olx',
