...
const ERRORS = {
  ...
  group_day_change_session_active: { status: 409, message: ({ reason } = {}) =>
                                `Не можна змінити день доставки або розклад під час активної сесії (${reason || 'цикл у процесі'}). ` +
                                'Поточні замовлення та збирання вже привʼязані до цієї сесії. ' +
                                'Змініть налаштування після завершення поточного циклу — вони застосуються до наступної сесії.' },
